...
- **그림자를 넣지 않는다** (2026-09-25). 쓰는 쪽이 \`box-shadow\`를 따로 얹지 않는다. 배경에 묻히면 그림자가 아니라 variant를 바꾼다(IconButton이면 \`outline\`).
...
**나란히 놓인 버튼 (2026-09-25)**
- 나란히 놓인 버튼은 따로 정한 것이 없으면 **늘 같은 크기(size)·같은 너비 정책**을 쓴다. 한쪽만 sm이거나, 한쪽만 꽉 채우고 한쪽은 내용만큼이면 안 된다
- 세로로 쌓여도 한 묶음이면 같다(떠 있는 [+]와 그 위 맨 위로). Button과 IconButton이 섞여도 같은 크기 단계를 쓴다(sm 36·md 48)
- 크기를 다르게 두려면 그 이유를 쓰는 쪽에 적는다

**너비 규칙**
- \`auto\`(내용만큼) 또는 컨테이너를 꽉 채우는 값만 쓴다. 고정 px 값을 주면 화면·컨테이너마다 버튼 크기가 따로 놀아 전체 일관성을 지키기 힘들어진다.

**링크 (2026-09-25)**
- 다른 페이지로 가는 단추는 \`href\`를 준다. 모양은 그대로이고 \`<a>\`로 그려진다(MenuItem의 href와 같다)
- onClick으로 주소를 바꾸지 않는다. 크롤러가 따라가지 못하고, 새 탭 열기·주소 복사가 안 된다
- 예: 카톡테마 메이커 템플릿 첫 화면의 공식 템플릿 [더보기]. Title의 action에 href를 준다(Title이 Button에 그대로 넘긴다). 사이트 안에서 공식 페이지로 가는 길이 이 링크다
