...
제목 아래 설명 한 줄. 머리 안에 들어가므로 본문을 내려도 함께 구르지 않는다.
창이 무엇을 하는 곳인지 한 줄로 말해야 하는데 그 말이 본문 첫 줄로 내려가 있으면, 목록을 내리는 순간 사라진다.
한 줄을 넘는 설명이나 주의사항은 본문에 둔다.

**toolbar**
머리 아래 늘 보이는 자리. 찾기칸·분류 탭처럼 목록을 내려도 손에 닿아야 하는 것을 둔다. 본문만 스크롤된다(이모지 고르기).

**footer**
본문 아래에 늘 보이는 단추 자리. 본문만 스크롤되고 footer는 창 바닥에 남는다. 입력이 길어도 확인 단추를 찾아 내려가지 않게 할 때 쓴다(내보내기). 여기 선 단추는 칸을 꽉 채운다(둘이면 반씩). 마지막 동작이라 넓어야 눈에 들고 폰에서 닿기도 쉽다. 쓰는 쪽에서 너비를 줄 필요가 없다.
