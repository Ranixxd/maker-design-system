...
| \`[10, 50, 90]\` | 끈 방향의 다음 스냅 포인트로 이동 (16px만 끌어도 넘어간다) |
...
#### 끄는 방향으로 스냅 (2026-09-18, 값은 2026-09-19에 한 번 더 민감하게)
\`snapPoints\`가 있으면 손잡이를 **조금만(16px) 끌어도** 끈 방향의 다음 스냅으로 넘어간다. 가장 가까운 스냅으로만 붙이면 반 넘게 끌어야 바뀌어 둔했다.
- 위로 끌면 더 높은 스냅으로, 아래로 끌면 더 낮은 스냅으로 간다. 가장 낮은 스냅에서 내리면 닫힌다(\`closeable\`일 때)
- 16px보다 덜 움직였으면 원래 스냅으로 돌아간다. **톡 치기로는 높이가 바뀌지 않는다**
- 손을 뗀 높이가 가장 낮은 스냅의 70%보다 낮으면 어느 스냅에서 시작했든 **한 번에 닫힌다**. 90%로 연 시트를 끝까지 내렸는데 50%에서 멈추면 두 번 끌어야 했다

#### 스냅 없는 시트 닫기 (2026-09-18, 값은 2026-09-19에 한 번 더 민감하게)
제 키의 20%(96px를 넘지 않게)만 아래로 끌어도 닫힌다. 덜 끌면 제자리로 돌아간다.
