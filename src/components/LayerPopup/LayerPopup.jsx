...
/* 이만큼(px)만 끌어도 끈 방향의 다음 스냅으로 넘어간다(2026-09-18).
   ...
   24px에서 16px로 한 번 더 낮췄고(2026-09-19), 2026-09-20에 8px로 내렸다
   (사용자 "살짝만 건드려도 붙게"). 톡 치기는 보통 2~6px 안에서 끝나므로 아직 높이가 바뀌지 않는다 */
const SNAP_SWIPE_PX = 8;

/* 스냅 없는 시트(내용 높이)를 닫는 끌기 거리. 제 키의 25%(120px 넘지 않게)만 내려도 닫힌다(2026-09-18).
   전에는 화면 높이 4.5% 아래까지 끌어야 닫혀서 "잘 안 내려간다"는 말을 들었다.
   2026-09-19에 20%(96px 넘지 않게)로 낮췄고, 2026-09-20에 12%(56px 넘지 않게)로 내렸다 */
const CLOSE_RATIO = 0.12;
const CLOSE_MAX_PX = 56;
/* 스냅 있는 시트에서 손을 뗀 높이가 가장 낮은 스냅의 이 비율보다 낮으면 한 번에 닫는다.
   90%로 열린 시트를 끝까지 끌어내렸는데 50%에서 멈추면 두 번 끌어야 했다(2026-09-18).
   2026-09-19에 0.6에서 0.7로, 2026-09-20에 0.85로 올려 덜 끌어도 닫힌다 */
const SNAP_CLOSE_RATIO = 0.85;
...
export default function LayerPopup({
  ...
  const onTouchEnd = useCallback(() => {
    ...
    /* 스냅 없는 시트(내용 높이). 제 키의 12%(최대 56px)만 내려도 닫고, 아니면 원래 높이로 돌아간다.
