...
\`<ul>\`을 그대로 넘긴다 — 점과 줄 간격은 부품이 그린다. 줄바꿈만 필요하면 \`<br />\`도 된다.

\`\`\`jsx
<TextField id="m_id" label="테마 고유 아이디" hint={(
  <ul>
    <li>자동으로 생성되며, 변경할 수 있어요.</li>
    <li>고유 아이디가 겹치면 기존 테마 파일이 덮어씌워져요.</li>
  </ul>
)} />
\`\`\`

칸 밖에 따로 세우지 않는다. 같은 설명을 자리만 다르게 두면 글자·색·간격이 갈라지고,
고치는 사람이 두 곳을 찾아다니게 된다. **MDS 칸이 아닌 자리**(카톡테마 메이커 내보내기 창의
아이콘처럼 직접 만든 칸)에만 따로 세우고, 그때도 생김새는 hint와 같게 맞춘다
(\`text-body-sm\`, secondary, 칸에서 8px).
