...
### 타이포 클래스는 새로 만들지 않는다 (2026-09-17)

`.text-heading-*` `.text-body-*` `.text-label-*` 열한 개 안에서 고른다. 딱 맞는 단계가 없어 보여도
가장 가까운 것을 쓴다. 사용자가 정한 것이다. `heading-xs`(작은 제목)는 같은 날 한 번 예외로
더했다. label을 제목처럼 쓰면 굵기가 빠져서다. 색·반경처럼 "없으면 제안하고 더한다"가
**타이포에는 해당하지 않는다.** 용도는 `typography.mdx`의 표에 있다.
...
- **MDX와 함께 `X.stories.jsx`도 읽는다.** 쓰는 쪽 규칙(늘어놓는 간격, 넘칠 때 처리,
  감싸는 요소가 할 일)이 거기 적혀 있다. **내가 만들어 둔 마크업을 부품으로 바꿔 끼우는 것도
  처음 쓰는 것에 든다** — 그때도 읽고, 부품이 이미 정한 내 CSS는 같이 걷는다
- **없는 부품을 새로 만들어야 할 것 같으면 먼저 사람에게 묻는다.** 부품이 하나 늘면
  두 서비스가 함께 이고 간다. 한 곳에서만 쓰는 것을 부품으로 올릴지는 사람이 정한다
